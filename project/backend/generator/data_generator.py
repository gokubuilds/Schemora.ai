    ...
    def _eval(node):
        if isinstance(node, ast.Name):
            ...
            try:
                ...
                if obj is faker_instance:
                    return getattr(base_faker, node.attr)
                raise
...
def generate_data(schema: dict, column_map: dict, topo_order: list, num_rows: int = 20, tables_config: dict = None) -> dict:
    """
    ...
    Any table not listed defaults to num_rows.
    """
    faker = Faker('en_IN')
    ...
    for table in topo_order:
        table_rows = tables_config.get(table, {}).get("rows", num_rows)
