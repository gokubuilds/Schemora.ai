document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPerTableInputs(tableNames) {
        ...
        perTableInputs.innerHTML = tableNames.map(name => `
            <div class="per-table-row" style="display: flex; justify-content: space-between; align-items: center; padding: 0.25rem 0;">
                <label for="rows-${name}" style="font-family: 'Space Grotesk', sans-serif; font-size: 0.85rem; font-weight: 500; color: var(--text-primary);">${name}</label>
                <input type="number" id="rows-${name}" data-table="${name}"
                       value="${defaultVal}" min="1" max="10000" class="per-table-input"
                       style="width: 80px; padding: 0.35rem 0.5rem; border: 1px solid var(--border-color); border-radius: 4px; background: rgba(255,255,255,0.05); color: var(--text-primary); font-family: 'JetBrains Mono', monospace; font-size: 0.85rem; text-align: right;">
            </div>
        `).join('');
        perTableSection.style.display = 'block';
    }

    // Re-render on DDL blur & input & load
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${baseUrl}/api/generate`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ddl, rows: defaultRows, tables_config })
            });
